...
//	private Mision mision;
	...
	private static final int WIDTH = 1400/2; // 2000 / 2;
	...
	private static final int SIMULATION_SPEED = 2;//10;    
	private Timer timer;
    private long t;
...
//		this.mision = mision;
		...
		this.step = step;
		this.t = tini;
//		if (this.bm.getRovers() != null && this.bm.getRovers().length > 0)
//			this.rover1 = this.bm.getRovers()[0];
//		else this.rover1 = new Rover("", new Vector (0,0), 0, new Vector(0,0), 0.0);
	}
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < this.bm.getMisiones().length; i++)
			this.paintMision(g, this.bm.getMisiones()[i]);
		for (int i = 0; i < this.bm.getRovers().length; i++)
			this.paintRover(g, this.bm.getRovers()[i]);
		g.setColor(c);
	}
