...
public class PruebaInteractiva3 {
	...
	public String run (String c) {
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "Rover"
			try {
				Rover nuevo = new Rover (fs[1], 
						this.pos,//new Vector(this.pos.getX(), this.pos.getY()),
						Long.parseLong (fs[2]),
						this.vel, //new Vector(this.vel.getX(), this.vel.getY()),
						Double.parseDouble(fs[3])); 
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "addVel"
			this.rover1.setVel(this.vel);//new Vector(this.vel.getX(), this.vel.getY()));
			return this.rover1.toString() + "\n";
		}
		i++;
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "mision"
			try {
				this.mision = new Mision (fs[1],
						this.rover1,
				//		new Rover(this.rover1.getId(), this.rover1.getPos(), this.rover1.getT(),
				//				this.rover1.getVel(), this.rover1.getPower()),
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "getmisiones"
			String r = "";
			for (Mision m: this.bm.getMisiones())
				r += m.toString();
			return r + "\n";
		}
		i++;
		...
		if (c.indexOf(cmds[i].split(" ")[0]) == 0) { // "sim"
			...
			SwingUtilities.invokeLater(new Runnable() {
	            public void run() {
	            	MapaPanel panel = new MapaPanel();
	                panel.init (bm, null, tini, tfin, 1); 
	                panel.createAndShowGUI();
	            }
	        });
			return "viewer launched";
